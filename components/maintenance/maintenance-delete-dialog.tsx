...
import { formatDate } from "@/lib/maintenance/format";

interface Props {
  record: MaintenanceWithRelations;
  pending: boolean;
  onClose: () => void;
  onConfirm: () => void;
}
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            >
              ยืนยันการลบรายการ Maintenance
            </h2>
            <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 p-3 text-sm">
              <div className="font-semibold text-gray-900">
                {record.maintenance_type} — {record.problem}
              </div>
              <div className="mt-1 text-xs font-medium text-gray-600">
                {record.machines
                  ? `${record.machines.machine_name} (${record.machines.machine_id})`
                  : "ไม่พบเครื่องจักร"}
                {" · "}
